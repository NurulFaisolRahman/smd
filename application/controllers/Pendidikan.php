...
class Pendidikan extends CI_Controller {
    ...
    //                   'Satuan' => $Volume,
		...
		if ($Gambar > 0) {
			for ($i=0; $i < $Gambar; $i++) { 
				// $NamaGambar = base_url("Pendidikan/".$_FILES['file'.$i]['name']);
				// $Tipe = pathinfo($Folder,PATHINFO_EXTENSION);
				// $valid_extensions = array("jpg","jpeg","png");
				// if( !in_array(strtolower($Tipe),$valid_extensions) ) {
				// $NamaGambar = date('Ymd',time()).rand(0,9999).'.jpg';
				print_r(move_uploaded_file($_FILES['file'.$i]['tmp_name'], "Pendidikan/".$_FILES['file'.$i]['name']));
			}
