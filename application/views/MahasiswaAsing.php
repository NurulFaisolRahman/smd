    ...
    <script src="<?=base_url('bootstrap/datatables/jquery.dataTables.js')?>"></script>
        ...
        $("#DownloadBorang").click(function() { 
          ...
          if (Pisah[1]-Pisah[0] >= 0) {
            ...
            var MahasiswaAsing = ['Pendidikan','Penelitian','Pengabdian']
					  for (let i = 0; i < 3; i++) {
              $.post(BaseURL+"Admin"+"/LampiranMahasiswaAsing/"+MahasiswaAsing[i]).done(function(Respon) {
                var array = JSON.parse(Respon) 
                var No = 1;
                array.forEach(function(object) {
                  $('#LampiranMahasiswaAsing').attr('href',BaseURL+'MahasiswaAsing/'+object.Bukti)		
                  $('#LampiranMahasiswaAsing').attr('Download','Kerja Sama '+MahasiswaAsing[i]+' '+No+'.pdf') 
                  $('#LampiranMahasiswaAsing')[0].click()
                  No+=1;
                })
              }) 	
            }
