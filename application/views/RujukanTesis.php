<div class="content-wrapper">
              ...
              <div class="col-sm-12 mt-2">
                <div class="container-fluid border border-warning rounded bg-light">
                  <div class="row align-items-center">
                    ...
                    <button type="button" class="btn btn-primary text-light mb-2" data-toggle="modal" data-target="#ModalInputRujukanTesis"><i class="fa fa-plus"></i> <b>Input Data</b></button> 
                      <div class="table-responsive mb-2">
                        <table id="TabelRujukanTesis" class="table table-bordered table-striped">
                          ...
                          <tbody>
                            <?php $No = 1; foreach ($RujukanTesis as $key) { ?>
                              <tr>	
                                ...
                                <td class="text-center align-middle">
                                  <button Edit="<?=$key['Id']."|".$key['Dosen']."|".$key['Tema']."|".$key['Mhs']."|".$key['Judul']."|".$key['Tahun']."|".$key['Bukti']?>" class="btn btn-sm btn-warning Edit"><i class="fas fa-edit"></i></button>
                                  <button Hapus="<?=$key['Id']."|".$key['Bukti']?>" class="btn btn-sm btn-danger Hapus"><i class="fas fa-trash"></i></button>  
                                  <button LihatSertifikat="<?=base_url('RujukanTesis/'.$key['Bukti'])?>" class="btn btn-sm btn-primary LihatSertifikat"><i class="fas fa-download"></i></button>  
                                </td> 
                              </tr>
                            <?php } ?>
                          </tbody>
    ...
    <div class="modal fade" id="ModalInputRujukanTesis">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content bg-warning">
          <div class="modal-body">
            <div class="container">
                ...
                <div class="col-sm-12">
                  <div class="input-group input-group-sm mb-2">
                    <div class="input-group-prepend">
                      <span class="input-group-text bg-primary"><b>Judul Tesis</b></span>
                    </div>
                    <input type="text" class="form-control" id="Judul" placeholder="Input Judul Tesis"> 
                  </div>
                </div>
                <div class="col-sm-8">
                  <div class="input-group input-group-sm mb-2">
          ...
          <div class="modal-footer justify-content-between">
            ...
            <button type="submit" class="btn btn-success" id="InputRujukanTesis"><b>Simpan</b></button>
          </div>
        </div>
      </div>
    </div>
    <div class="modal fade" id="ModalEditRujukanTesis">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content bg-warning">
          <div class="modal-body">
            <div class="container">
                ...
                <div class="col-sm-12">
                  <div class="input-group input-group-sm mb-2">
                    ...
                    <input type="text" class="form-control" id="EditJudul" placeholder="Input Judul Tesis"> 
                  </div>
                </div>
                <div class="col-sm-8">
                  <div class="input-group input-group-sm mb-2">
                    ...
                    <div class="input-group-prepend">
                      <button class="input-group-text bg-primary" id="CancelRujukanTesis"><b>X</b></button>
                    </div>
                  </div>
                  <pre class="text-danger"><b>Bukti Yang Diupload Dalam Format Pdf</b></pre>
                </div>
          ...
          <div class="modal-footer justify-content-between">
            <button type="button" class="btn btn-danger" data-dismiss="modal"><b>Tutup</b></button>
            <button type="submit" class="btn btn-success" id="UpdateRujukanTesis"><b>Simpan</b></button>
          </div>
        </div>
      </div>
    </div>
    ...
    <script src="<?=base_url('bootstrap/js/Borang.js')?>"></script>
        ...
        $("#CancelRujukanTesis").click(function() {
					$("#EditBukti").val("")
				})

        $("#InputRujukanTesis").click(function() {
          if ($("#Dosen").val() == "") {
            ...
            $.ajax({
							url: BaseURL+'Admin/InputRujukanTesis',
							type: 'post',
							data: fd,
							contentType: false,
							processData: false,
							success: function(Respon){
								if (Respon == '1') {
									window.location = BaseURL + "Admin/RujukanTesis"
								}
								else {
									alert(Respon)
								}
        ...
        $(document).on("click",".Edit",function(){
          ...
          $('#ModalEditRujukanTesis').modal("show")
				}) 

				$("#UpdateRujukanTesis").click(function() {
					if ($("#EditDosen").val() == "") {
            alert('Mohon Input Nama Dosen!')
            ...
            alert('Mohon Input Judul Tesis!')
          } else if (isNaN($("#EditTahun").val()) || $("#EditTahun").val() == "") {
            alert('Input Tahun Belum Benar!')
          } else {
            var fd = new FormData()
            fd.append('Id',$("#Id").val())
            ...
            $.ajax({
							url: BaseURL+'Admin/UpdateRujukanTesis',
							type: 'post',
							data: fd,
							contentType: false,
							processData: false,
        ...
        $(document).on("click",".Hapus",function(){
      		...
      		if (Konfirmasi == true) {
						$.post(BaseURL+"Admin/HapusRujukanTesis", Hapus).done(function(Respon) {
							if (Respon == '1') {
								window.location = BaseURL + "Admin/RujukanTesis"
							} else {
								alert(Respon)
